...
# -*- coding: utf-8 -*-
"""
Manufactured phase-separation-like evolution on the corrected crescent domain.

This script keeps the same crescent geometry and Section-3 homogeneous Neumann
constraint framework, but switches to an Allen-Cahn-type manufactured problem
whose exact solution is designed to resemble spinodal decomposition /
oil-water-separation-like coarsening.
"""
...
def q_np(x, y):
    qout = Ro * Ro - (x * x + y * y)
    qin = (x - dc) * (x - dc) + y * y - Ri * Ri
    return qout * qin


def q_torch(x, y):
    return (Ro * Ro - (x * x + y * y)) * (((x - dc) * (x - dc) + y * y) - Ri * Ri)


def mask_fn(X, Y):
    return ((X * X + Y * Y) <= Ro * Ro) & (((X - dc) * (X - dc) + Y * Y) >= Ri * Ri)


def crescent_boundaries(nb_outer: int, nb_inner: int):
    th = np.linspace(0.0, 2.0 * np.pi, 4 * nb_outer, endpoint=False)
    xo = Ro * np.cos(th)
    yo = Ro * np.sin(th)
    keep = ((xo - dc) ** 2 + yo**2) >= Ri**2 - 1e-12
    xo = xo[keep]
    yo = yo[keep]
    ids = np.linspace(0, len(xo) - 1, min(nb_outer, len(xo)), dtype=int)
    xo = xo[ids]
    yo = yo[ids]
    nxo = xo / Ro
    nyo = yo / Ro

    ph = np.linspace(0.0, 2.0 * np.pi, 4 * nb_inner, endpoint=False)
    xi = dc + Ri * np.cos(ph)
    yi = Ri * np.sin(ph)
    keep = (xi * xi + yi * yi) <= Ro**2 + 1e-12
    xi = xi[keep]
    yi = yi[keep]
    ids = np.linspace(0, len(xi) - 1, min(nb_inner, len(xi)), dtype=int)
    xi = xi[ids]
    yi = yi[ids]
    nxi = -(xi - dc) / Ri
    nyi = -yi / Ri
    ...
    nx = np.concatenate([nxo, nxi])
    ny = np.concatenate([nyo, nyi])
    return x, y, nx, ny, (xo, yo), (xi, yi)


def exact_u_torch(x, y, t, amp: float, omega: float, sharpness: float, coarsen_rate: float):
    """Phase-separation-like exact field with fine-to-coarse morphology change."""
    q = q_torch(x, y)
    q2 = q * q
    phase = omega * t
    coarse_w = 1.0 - torch.exp(-coarsen_rate * t)
    fine_w = torch.exp(-0.65 * coarsen_rate * t)

    shift_x = 0.08 * torch.sin(0.55 * phase) - 0.05 * torch.cos(1.10 * phase)
    shift_y = -0.07 * torch.cos(0.45 * phase) + 0.05 * torch.sin(1.35 * phase)
    xp = x - shift_x
    yp = y - shift_y

    fine1 = torch.sin(4.8 * np.pi * xp + 0.65 * phase) * torch.sin(4.2 * np.pi * yp - 0.70 * phase)
    fine2 = torch.cos(5.4 * np.pi * (xp + 0.25 * yp) - 0.45 * phase) * torch.sin(3.8 * np.pi * (yp - 0.20 * xp) + 0.80 * phase)
    fine3 = torch.sin(6.2 * np.pi * (xp - 0.10 * yp) + 0.35 * phase)

    coarse1 = torch.sin(1.45 * np.pi * (x + 0.32 * y) + 0.30 * phase)
    coarse2 = torch.cos(1.15 * np.pi * (y - 0.18 * x) - 0.42 * phase)
    coarse3 = torch.sin(0.95 * np.pi * (0.8 * x - 1.1 * y) + 0.55 * phase)

    c1x = -0.40 + 0.18 * torch.cos(0.80 * phase)
    c1y = -0.12 + 0.16 * torch.sin(0.95 * phase)
    blob1 = torch.exp(-18.0 * ((x - c1x) ** 2 + (y - c1y) ** 2))

    c2x = -0.18 + 0.14 * torch.sin(1.05 * phase)
    c2y = 0.30 * torch.cos(0.70 * phase)
    blob2 = torch.exp(-16.0 * ((x - c2x) ** 2 + (y - c2y) ** 2))

    c3x = -0.52 + 0.10 * torch.cos(1.20 * phase)
    c3y = 0.42 * torch.sin(0.75 * phase)
    blob3 = torch.exp(-20.0 * ((x - c3x) ** 2 + (y - c3y) ** 2))

    fine_mix = 0.52 * fine1 + 0.34 * fine2 + 0.18 * fine3
    coarse_mix = 0.92 * coarse1 - 0.78 * coarse2 + 0.48 * coarse3
    droplet_mix = 1.15 * blob1 - 0.95 * blob2 + 0.80 * blob3
    order = amp * (0.95 * fine_w * fine_mix + 1.30 * coarse_w * coarse_mix + 0.90 * coarse_w * droplet_mix)
    return torch.tanh(sharpness * q2 * order)


def exact_and_force(x_np, y_np, t_scalar, *, eps2, mu, beta, amp, omega, sharpness, coarsen_rate, device):
    ...
    u = exact_u_torch(x, y, t, amp, omega, sharpness, coarsen_rate)
...
def parse_args() -> Args:
    p = argparse.ArgumentParser(
        description="Crescent-domain manufactured phase-separation-like Allen-Cahn rollout."
    )
    ...
    p.add_argument("--coarsen_rate", type=float, required=True)
...
def main():
    ...
    xb, yb, nxb, nyb, outer, inner = crescent_boundaries(args.Nb_outer, args.Nb_inner)
    xd, yd, nxd, nyd, outerd, innerd = crescent_boundaries(args.Nb_dense, args.Nb_dense)
    dpx_b, dpy_b = eval_trig_basis_grad_from_meta(xb, yb, meta)
    C = nxb[:, None] * dpx_b + nyb[:, None] * dpy_b
    dpx_d, dpy_d = eval_trig_basis_grad_from_meta(xd, yd, meta)
    C_dense = nxd[:, None] * dpx_d + nyd[:, None] * dpy_d
    ...
    plot_domain(
        grid,
        os.path.join(args.outdir, "domain.png"),
        [(outerd[0], outerd[1], "outer Neumann"), (innerd[0], innerd[1], "inner Neumann")],
    )
    ...
    u_init, _ = exact_and_force(
        ...
        coarsen_rate=args.coarsen_rate,
        device=device,
    )
    ...
    def rhs_fn(zv: np.ndarray, t: float) -> np.ndarray:
        ...
        rhs = args.eps2 * lap + args.mu * u - args.beta * u**3 + f
    ...
    boundaries = [(outerd[0], outerd[1]), (innerd[0], innerd[1])]
    ...
    plot_error_curve(
        ...
        "crescent manufactured phase separation",
    )
