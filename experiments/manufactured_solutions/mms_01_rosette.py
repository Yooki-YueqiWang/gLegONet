...
# -*- coding: utf-8 -*-
"""
Section-3 manufactured-solution benchmark on the corrected six-petal rosette.

This variant keeps the same rosette domain and cubic reaction-diffusion
equation as the corrected script, but uses a more dynamic manufactured
solution so that rollout snapshots show visible temporal change.
"""
...
def re_z6_np(x, y):
    return x**6 - 15 * x**4 * y**2 + 15 * x**2 * y**4 - y**6


def q_np(x, y):
    r2 = x * x + y * y
    return 0.53**6 - r2**3 + 0.72 * re_z6_np(x, y)


def q_torch(x, y):
    r2 = x * x + y * y
    re6 = x**6 - 15 * x**4 * y**2 + 15 * x**2 * y**4 - y**6
    return 0.53**6 - r2**3 + 0.72 * re6
...
def boundary_rosette(nb: int):
    th = np.linspace(0.0, 2.0 * np.pi, nb, endpoint=False)
    r0 = 0.53
    alpha = 0.72
    R = r0 / np.maximum(1.0 - alpha * np.cos(6.0 * th), 1e-12) ** (1.0 / 6.0)
    x = R * np.cos(th)
    y = R * np.sin(th)
...
def exact_u_torch(x, y, t, amp: float, omega: float):
    """Dynamic manufactured solution with the same homogeneous Dirichlet factor."""
    q = q_torch(x, y)
    phase = omega * t
    mix1 = torch.sin(0.65 * phase)
    mix2 = torch.cos(1.10 * phase + 0.35)
    envelope = 1.05 + 0.35 * torch.sin(0.40 * phase) + 0.25 * torch.cos(1.25 * phase)

    mode1 = torch.sin(2.0 * np.pi * x + 1.00 * phase) * torch.cos(np.pi * y - 0.45 * phase)
    mode2 = torch.cos(np.pi * x - 0.75 * phase) * torch.sin(3.0 * np.pi * y + 0.90 * phase)
    mode3 = torch.sin(2.0 * np.pi * (x + 0.40 * y) - 0.55 * phase) * torch.sin(np.pi * (x - 2.10 * y) + 0.70 * phase)
    mode4 = torch.cos(3.0 * np.pi * x + 0.35 * phase) * torch.cos(2.0 * np.pi * y - 1.20 * phase)

    w = amp * envelope * (
        (0.45 + 0.40 * mix1) * mode1
        + (0.30 + 0.45 * mix2) * mode2
        + 0.34 * torch.sin(0.95 * phase) * mode3
        + 0.22 * torch.cos(1.55 * phase) * mode4
    )
    return q * w


def exact_and_force(x_np, y_np, t_scalar, *, eps2, mu, beta, amp, omega, device):
...
def parse_args() -> Args:
    p = argparse.ArgumentParser(
        description="MMS-I corrected rosette cubic reaction-diffusion with visibly dynamic rollout."
    )
...
def main():
    ...
    xb, yb, _, _ = boundary_rosette(args.Nb)
    xd, yd, _, _ = boundary_rosette(args.Nb_dense)
    ...
    u_init, _ = exact_and_force(
        x_in, y_in, 0.0, eps2=args.eps2, mu=args.mu, beta=args.beta, amp=args.amp, omega=args.omega, device=device
    )
    ...
    def rhs_fn(zv: np.ndarray, t: float) -> np.ndarray:
        ...
        _, f = exact_and_force(
            x_in, y_in, t, eps2=args.eps2, mu=args.mu, beta=args.beta, amp=args.amp, omega=args.omega, device=device
        )
    ...
    for k, t in enumerate(times):
        ...
        ref, _ = exact_and_force(
            x_in, y_in, float(t), eps2=args.eps2, mu=args.mu, beta=args.beta, amp=args.amp, omega=args.omega, device=device
        )
    ...
    plot_error_curve(
        ...
        "MMS-I corrected rosette cubic reaction-diffusion",
    )
    plot_scalar_snapshots(
        ...
        (xd, yd),
        "MMS-I corrected dynamic: Section 3 vs exact",
    )
    plot_change_from_initial(
        ...
        (xd, yd),
        "MMS-I corrected dynamic: visible change from initial state",
    )
    plot_curve(
        times,
